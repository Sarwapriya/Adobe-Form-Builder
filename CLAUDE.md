...
- **One page per language.** Each of a form's locales gets its own HTML + behavior-JS pair per variant and its own stylesheet, while the one main data file is shared by every language — `2·languages·variants + languages + 1` files. Naming (`fileNames.ts`, mirrored in `file_names.py`): HTML/behavior JS `<Sub>-<lang>_<projectCode>_<FF|OC>.html`/`.js` (e.g. `SESAR-EN_F2H26_FF.html`), main data JS `<Sub>_<projectCode>.js` (`SESAR_F2H26.js`), CSS `<lang>-<projectCode>.css` (`EN-F2H26.css`); `<lang>` is the upper-cased language subtag (falling back to `EN-GB`/`EN-US` when two locales share a language), and the project-code segment is omitted while a form has no project code. The project code isn't part of `FormDefinition` — it's `BuilderConfig.projectCode`, injected at generation time from `Form.projectCode` (`form_builder_service.publish_form`), so it can be assigned after the draft exists. Each language's behavior JS starts with a one-line `languagePin` making that language the page's default (the reference script below stays byte-identical after it), and `?lang=<localeCode>` still switches any page to another language.
...
There is no self-service signup — `POST /api/v1/admin/users` (admin-or-superadmin, optionally takes `subsidiaryId`, required when `role: "standard"`) is the only way to create an account, plus the one-time `scripts/seed_admin.py` for the very first admin. A plain `"admin"` may only provision/enable/disable `"standard"` accounts; only a `"superadmin"` may provision or toggle another `"admin"`/`"superadmin"` account (checked inline in `app/routers/admin.py`, since `require_admin` alone isn't fine-grained enough) — and nobody may disable their own account. `DELETE /api/v1/admin/users/{id}` follows the same authorization split and is a **soft delete** (see "Soft delete" below) — the account is flagged `isDeleted`, disabled, hidden, and its sessions revoked, however much history it has.

### Soft delete (nothing the UI calls "Delete" removes a row)

Every user-facing "Delete" is a status update, not a `DELETE FROM`: forms (`Form.isDeleted`, always — `form_builder_service.delete_form`), users (`User.isDeleted`/`deletedAt`, also `isActive = False`, refresh tokens revoked, and `username`/`emailHash` tombstoned as `<original>~deleted~<id8>` so the unique constraints free the login for reuse), subsidiaries (`Subsidiary.isDeleted`, also inactive; re-adding the name *restores* the same row) and subsidiary locales (`SubsidiaryLocale.isDeleted`; re-adding the code restores it). Every list/lookup filters `isDeleted == False` — when adding a query over one of these tables, do the same. LLMs are never deleted at all: `FabrixModel` and `AiProvider` are only enabled/disabled (there is no delete route). The one remaining real delete is `SubsidiaryProjectBlock`, because that row's *presence* is the on/off switch (a toggle, not an item being deleted). The frontend asks for confirmation first (`ConfirmDialog`, or `useConfirm()` in `src/hooks/useConfirm.tsx` as a promise-based `window.confirm` replacement). The schema for this lives in `backend-py/scripts/soft_delete_and_ai_providers_migration.sql` (idempotent) — there is no migration framework, so run it by hand against each database.

### AI assistant providers (`app/services/aiProviderService.py`, `fabrixAIService.py`, `openaiCompatAIService.py`, `ai_providers_service.py`)

**FabriX** (an internal OpenAPI chat endpoint, `app/services/fabrix_settings_service.py`) always gets first priority. After it, every enabled **"other" provider** is tried in listed order until one answers: any number of them, each an admin-named row in `fq.AiProviders` (`AiProvider` model — name, `baseUrl` up to but excluding `/chat/completions`, model, encrypted `apiKeyEnc`, `isEnabled`, `sortOrder`), managed from Configuration > AI Assistant > Other AI Providers. Each is just an OpenAI-compatible chat-completions endpoint (Groq, OpenAI, a gateway, ...); the Groq-only `reasoning_effort` tweak is sent only to `*.groq.com`. If the table has no rows at all, a Groq provider is synthesized from the legacy `GROQ_*` env vars. There is no "Claude/Anthropic" tier. Every outbound-error string from any provider is sanitized to a generic customer-facing message before it ever reaches the AI assistant chat UI (`aiAssistantService.GENERIC_AI_FAILURE_MESSAGE`) — raw provider errors are logged server-side only.
