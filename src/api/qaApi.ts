...
/** GET /api/v1/admin/qa-runs?formId= — every QA run ever triggered for one
 * Configuration form (covers both contribution-based and ad-hoc
 * pending-review runs for that form), newest first. */
export function listQaRuns(subject: QaRunSubject): Promise<QaRun[]> {
  return apiClient.get<QaRun[]>(`/api/v1/admin/qa-runs?formId=${encodeURIComponent(subject.formId)}`);
}
