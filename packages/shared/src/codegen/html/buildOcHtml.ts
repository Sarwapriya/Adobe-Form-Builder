...
export function buildOcHtml(form: FormDefinition, config: BuilderConfig, fileNames: FileNames, locale?: LocaleCode): GeneratedFile {
  return { path: languageFileNames(fileNames, locale).ocHtml, contents: renderPage(form, config, "oc", fileNames, locale) };
}
